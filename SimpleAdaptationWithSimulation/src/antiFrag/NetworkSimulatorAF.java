...
public class NetworkSimulatorAF extends DeltaIoTSimulator {
    ...
    public static Simulator createSimulatorCase1noProbs(int x, int y, double battery16, int load16, int neighId, double delta){
        /*
        case 1 = adding motes to the edge of the network with a lot of battery
         */
        Simulator simul = new Simulator();
        ...
        mote14.addLinkTo(mote12,  gateway, power, distribution);
        mote15.addLinkTo(mote12,  gateway, power, distribution);
        //16 connects to its very neighbour, and we only know its id
        ...
        // Set profiles for some links and motes
        mote2 .setActivationProbability(new Constant<>(1.00));
        mote8 .setActivationProbability(new Constant<>(1.00));
        mote10.setActivationProbability( new Constant<>(1.00));
        mote13.setActivationProbability(new Constant<>(1.00));
        mote14.setActivationProbability(new Constant<>(1.00));

        mote12.getLinkTo(mote3).setInterference(new Constant<>(0.00));

        // Add SNR equations (from Usman's settings class)
        double mult = 0.0;
        mote2 .getLinkTo(mote4  ).setSnrEquation(new SNREquation( mult,		0));
        mote3 .getLinkTo(gateway).setSnrEquation(new SNREquation( mult,		 0));
        mote4 .getLinkTo(gateway).setSnrEquation(new SNREquation( mult,		0));
        mote5 .getLinkTo(mote9  ).setSnrEquation(new SNREquation( mult,		 0));
        mote6 .getLinkTo(mote4  ).setSnrEquation(new SNREquation( mult,		0));
        mote7 .getLinkTo(mote3  ).setSnrEquation(new SNREquation( mult,		 0));
        mote7 .getLinkTo(mote2  ).setSnrEquation(new SNREquation( mult,		 0));
        mote8 .getLinkTo(gateway).setSnrEquation(new SNREquation( mult,		 0));
        mote9 .getLinkTo(gateway).setSnrEquation(new SNREquation( mult,		 0));
        mote10.getLinkTo(mote6  ).setSnrEquation(new SNREquation( mult,		0));
        mote10.getLinkTo(mote5  ).setSnrEquation(new SNREquation( mult,		0));
        mote11.getLinkTo(mote7  ).setSnrEquation(new SNREquation( mult,		0));
        mote12.getLinkTo(mote7  ).setSnrEquation(new SNREquation( mult,		 0));
        mote12.getLinkTo(mote3  ).setSnrEquation(new SNREquation( mult,		0));
        mote13.getLinkTo(mote11 ).setSnrEquation(new SNREquation(mult,		0));
        mote14.getLinkTo(mote12 ).setSnrEquation(new SNREquation( mult,		 0));
        mote15.getLinkTo(mote12 ).setSnrEquation(new SNREquation( mult,		 0));
        mote16.getLinkTo(allMotes[neighId-2] ).setSnrEquation(new SNREquation( mult,		 0));

        // Global random interference (mimicking Usman's random interference) simul.getRunInfo().setGlobalInterference(new DoubleRange(-5.0, 5.0));
        simul.getRunInfo().setGlobalInterference(new DoubleRange(0.0,0.0));
    ...
    public static Simulator createSimulatorCase2noProbs(int[] x, int[] y, double battery16, int load16, int[] neighId, double delta){
        /*
        unknown = adding 2 motes to the edge of the network with a lot of battery
         */
        ...
        mote14.getLinkTo(mote12 ).setSnrEquation(new SNREquation( mult,		 0));
        mote15.getLinkTo(mote12 ).setSnrEquation(new SNREquation( mult,		 0));
        mote16.getLinkTo(allMotes[neighId[0]-2] ).setSnrEquation(new SNREquation( mult,		 0));
        mote17.getLinkTo(allMotes[neighId[1]-2] ).setSnrEquation(new SNREquation( mult,		 0));
